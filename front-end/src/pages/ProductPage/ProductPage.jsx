...
const ProductPage = () => {
    ...
    const [allImages] = React.useState([
        ...
        {id:'img2', class:'hue-rotate-180 border-[2px] border-gray-200 rounded-[18px]'},
        {id:'img3', class:'grayscale border-[2px] border-gray-200 rounded-[18px]'},
    ])
    ...
    // colors selector
    const [seColors] = React.useState([{name:'col1', class:'bg-yellow-900'}, {name:'col2', class:'bg-emerald-900'}, {name:'col3', class:'bg-indigo-900'}])
    ...
    // size selector
    const [sizeSel] = React.useState([{name:'Small'}, {name:'Medium'}, {name:'Large'}, {name:'X-Large'}])
        ...
        <div className="pt-[100px] pb-[100px] px-[20px] relative">
            ...
            <div className="flex flex-col justify-center items-center gap-[12px] lg:gap-[24px] lg:flex-row lg:items-start end:w-[1500px]">
                ...
                <div className="flex flex-col justify-start items-start lg:items-start gap-[4px] md:justify-center md:items-center lg2:w-[800px]">
                    <p className="text-[24px] text-black font-bold lg:text-[40px]">{product.title}</p>
                    <p>{"⭐".repeat(product.rate)} {product.rate}/5</p>
                    <p className="text-[24px] text-black font-bold">${product.price} {product.downPrice !== 0 ? (<del className="text-gray-500">${product.downPrice}</del>)
                    : null}</p>
                    <p className="text-[16px] text-gray-500">{product.title.repeat(5)} Lorem ipsum dolor sit amet consectetur adipisicing elit. Aspernatur, exercitationem?</p>
            ...
            <div className="mt-[20px] end:w-[1500px]">
                ...
                <div className="w-[100%] flex flex-row justify-center items-center mt-[10px]">
                    <button className="flex fle-row justify-center items-center w-[250px] px-[20px] py-[8px] border-[1px] border-gray-300 rounded-[24px]
                    hover:border-red-600 transition-all duration-[0.3s]">Lead More Reviews</button>
                </div>
            </div>
            </div>
                ...
                <div className="flex flex-row justify-center items-center w-[100%] end:w-[1500px]">
                    ...
                    >
                    {
                        randomProducts.slice(0, 4).map((rdProduct) => {
                            const rdImage = images[`../../public/assets/images/${rdProduct.image}`]
                            return (
                                <SwiperSlide key={rdProduct.id}>
                                <Product key={rdProduct.id} product={rdProduct} imgSrc={rdImage} />
                                </SwiperSlide>
                            )
                        })
                    }
